...
def txt_help():
    msg = """感謝使用 Bot of LazyRC Inc. 目前支援三種模式（請輸入切換模式）
1.查詢模式
1a. 公車查詢 (輸入：<公車號碼><站牌名稱>, 例: 650喬治商職)
1b. 天氣（輸入：<台灣縣市>, 例：台北）
1c. 星座運勢（輸入：<星座>, 例：射手）
1d. 匯率（輸入：<幣別>, 例：美金）
1e. 若查詢不到，則會以 google search 第一筆搜尋結果為答案
2.台鐵訂票模式（系統會幫您訂票，再請自行使用台鐵付款方式）
3.高鐵訂票模式（系統會幫您訂票，再請自行使用高鐵付款方式）"""
