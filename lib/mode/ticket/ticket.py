...
class TicketDB(DB):
    ...
    def schedule_waitting_ticket(self, user_id, ticket_type, parent_id, trainno):
        ...
        if ticket_type == TRA:
            del ticket["getin_start_dtime"]
            del ticket["getin_end_dtime"]
    ...
    def non_booking(self, ticket_type, status=TICKET_STATUS_SCHEDULED):
        ...
        sql = "SELECT user_id, ticket, retry, id, parent_id FROM {} WHERE token = '{}' AND ticket_number = '-1' AND {} BETWEEN '{}' AND '{}' AND status = '{}' AND ticket_type = '{}'".format(\
        ...
        return [(row[0], json.loads(row[1]), row[2], row[3], row[4]) for row in self.select(sql)]
